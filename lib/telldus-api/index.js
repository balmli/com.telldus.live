...
class Api {
    ...
    async listDevices({ telldusClientId, deviceType }) {
        const response = await this.request({
            path: '/devices/list',
        ...
        return response.device
            ...
            .filter(d => !deviceType || deviceType.id === d.deviceType)
            .map(d => {
                return {
                    ...d,
                    deviceTypeDescr: d.deviceType ? constants.DEVICE_TYPES_INVERTED[d.deviceType] : undefined
                }
            });
    }
